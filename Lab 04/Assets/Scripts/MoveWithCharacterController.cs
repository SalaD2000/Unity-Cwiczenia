...
public class MoveWithCharacterController : MonoBehaviour
{
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        if(hit.gameObject.tag == "Kontakt_Lab6")
        {
            Debug.Log("Chopie ty zes blocka dotknal ni?");
        }

    }

    public void Skok(float jump)
    {
        float a = jumpHeight;
        playerVelocity.y = Mathf.Sqrt(jump * a * -3.0f * gravityValue);
    }

    // private void OnControllerColliderHit(ControllerColliderHit other)
    // {
    //     if(other.gameObject.tag == "Kontakt_Lab6")
    //     {
    //         Debug.Log("Chopie ty zes blocka dotknal ni?");
    //     }
    // }
